...
func GET(c *gin.Context) {
    ...
    qspid := c.Param("pid")
    qpid, err := hex.DecodeString(qspid)
    if err != nil {
    ...
    rows, err := dbc.Query("SELECT ID, LastName, FirstName, MiddleName, Birthyear, Arrests, Weight, Height, Gender, Race FROM people WHERE ID=?", qpid)
    ...
    arrest_rows, err := dbc.Query("SELECT ID, PID FROM arrests WHERE PID=?", qpid)
